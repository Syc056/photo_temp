...
import { getAudio, getPhotoForQrDownload } from '../api/config';
...
function Landing() {
     ...
     const savePhotoBackendQr = async () => {
          const response = await getPhotoForQrDownload(uuid);
          if (response && response.images_list && response.images_list.length > 0) {
               let buildedUrl = `${process.env.REACT_APP_SHARE}/?code=${response.tracking_code}`;
               setQrUrl(buildedUrl);
          }
